...
pub fn user_idle_seconds() -> i32 {
    // Простой измерить нечем — отдаём заведомо большое значение, как заглушка:
    // ноль означал бы «человек всё время за мышью», и уступание превращалось бы
    // в паузу перед каждым действием, после которой агент всё равно действует.
